...
def request(method, url, **kwargs):
    ...
    logger.debug('curl -X {} {}'.format(response.request.method, response.url))
    return response


def get(url, params=None, **kwargs):
    response = originals['get'](url, params=params, **kwargs)
    logger.debug('curl -L {}'.format(response.url))
    return response


def options(url, **kwargs):
    response = originals['options'](url, **kwargs)
    logger.debug('curl -L -X OPTIONS {}'.format(response.url))
    return response


def head(url, **kwargs):
    response = originals['head'](url, **kwargs)
    logger.debug('curl -L -X HEAD {}'.format(response.url))
    return response
...
def post(url, data=None, json=None, **kwargs):
    ...
    logger.debug('curl -X POST {} {}'.format(data_option, response.url))
    return response
...
def put(url, data=None, **kwargs):
    ...
    if data:
        data_option = "--data-urlencode '{}'".format(urllib.parse.urlencode(data))
    logger.debug('curl -X PUT {} {}'.format(data_option, response.url))
    return response
...
def patch(url, data=None, **kwargs):
    ...
    if data:
        data_option = "--data-urlencode '{}'".format(urllib.parse.urlencode(data))
    logger.debug('curl -X PATCH {} {}'.format(data_option, response.url))
    return response


def delete(url, **kwargs):
    response = originals['delete'](url, **kwargs)
    logger.debug('curl -X DELETE {}'.format(response.url))
    return response
